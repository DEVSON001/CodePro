...
local currentFarmMode = nil -- "normal", "haki"
...
local function activateHaki()
    ...
    remoteSkill:FireServer(unpack(args))
    task.wait(0.5)
end
...
-- เริ่มใหม่เมื่อ respawn
player.CharacterAdded:Connect(function()
    if autoFarmEnabled and currentFarmMode then
        task.wait(1.5)
        equipWeapon()
        if currentFarmMode == "normal" then
            startFarm(false)
        elseif currentFarmMode == "haki" then
            startFarm(true)
        end
    end
end)
